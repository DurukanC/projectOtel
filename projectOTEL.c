...
typedef struct{
	...
	int odaNumarasi;//1-100 arasinda.
	double fiyatBilgisi;
...
void yeniMusteriEkleme(odaBilgileri *odalar){
	...
	odalar[sorgulanacakOda-1].odaNumarasi=sorgulanacakOda;//ODA NUMARASİ ATANDİ.
	...
	printf("Lutfen kalacak kisi sayisini giriniz:");
	...
	printf("Odemeniz gereken toplam miktar:%.2lfTL\n",odalar[sorgulanacakOda-1].fiyatBilgisi);
	...
	odalar[sorgulanacakOda-1].odaDurumu=1;//yani odamiz doldu.
}
...
void fiyatBilgisiYazdirma(){
	...
	printf("Lutfen kac kisi oalcaginizi giriniz:");
	scanf("%d",&kisiSayisi);
	...
	printf("Odemeniz gereken toplam tutar:%.2lf\n",toplamTutar);
...
void yedekKayitAlma(odaBilgileri *odalar){
	...
	for(i=0;i<ODA_SAYISI;i++){
		if(odalar[i].odaDurumu!=0){
			...
			fprintf(otelKayitlari,"Toplam Tutar:%2.lfTL\n",odalar[i].fiyatBilgisi);
...
int main(){
	...
	odaDizisi=(odaBilgileri*)calloc(ODA_SAYISI,sizeof(odaBilgileri));//dinamik şekilde oluşturuldu.
